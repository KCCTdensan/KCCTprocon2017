...
def read_QR():
    ...
    f=[]
    for file in glob.glob("C:\\Users\\admin\\Desktop\\hint\\IMG_*.JPG"):
    ...
    for pieces_data in f:
        ...
        for piece_data_str in pieces_data.split(":")[1:]:
                ...
                print(vertexes)
                pieces.append(vertexes)
        print("count",len(pieces))
    return pieces

class GUI():
    def draw_piece(self,piece,zoom,x,y):
        self.canvas.create_polygon([[(p[0] +10) * zoom,p[1] * zoom+y] for p in piece],fill="#9999FF",outline="black")

    def draw(self):
        zoom = self.zoom_scale.get()
        self.canvas.delete(tkinter.ALL)
        for piece in self.problem:
            self.draw_piece(piece,zoom,0,0)
    ...
    def __init__(self,problem):
        ...
        self.zoom_scale = tkinter.Scale(self.root, label = '倍率', orient = 'h',
               from_ = 1, to = 20, command = self.draw_handler,resolution=0.1)
